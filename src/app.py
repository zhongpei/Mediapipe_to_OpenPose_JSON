    ...
    def uigroup(self, tabname, is_img2img, elem_id_tabname):
        ...
        if shared.opts.data.get("controlnet_disable_control_type", False):
            ...
            with gr.Row(elem_classes="controlnet_control_type"):
                type_filter = gr.Radio(list(preprocessor_filters.keys()), label=f"Control Type", value='All',
        ...
        if gradio_compat:
            ...
            if type_filter is not None:
                def filter_selected(k, pp):
                    ...
                    filtered_preprocessor_list = [x for x in preprocessor_list if
                                                  pattern in x.lower() or x.lower() == 'none']
                    ...
                    return [gr.Dropdown.update(value=default_option, choices=filtered_preprocessor_list),
                            gr.Dropdown.update(value=default_model, choices=filtered_model_list)] + build_sliders(
                        default_option, pp)

                type_filter.change(filter_selected, inputs=[type_filter, pixel_perfect],
                                   outputs=[module, model, processor_res, threshold_a, threshold_b, advanced, model,
                                            refresh_models])
